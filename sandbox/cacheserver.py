...
def handleQuery(query, socket):
	query = json.loads(query)
	if query['operation'] == "getObjects":
		response = json.dumps(objects)
		socket.send(response)
	elif query['operation'] == "getMapExtents":
		response = json.dumps([starmap.map_lower_left, starmap.map_upper_right])
		socket.send(response)
...
if __name__ == "__main__":
	...
	for obj in cache.objects.values():
		objects[obj.id] = starmap.getScaledPosition(obj.pos)

	while True:
		handleQuery(socket.recv(), socket)
